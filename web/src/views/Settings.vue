...
async function changePassword() {
  ...
  try {
    ...
    newPassword.value = ''
    pwMessage.value = 'Passwort geändert. Alle anderen Sitzungen wurden beendet.'
  } catch (err) {
...
<template>
  <div class="fade-in max-w-2xl px-8 py-6">
    <h1 class="mb-5 text-[18px] font-semibold tracking-tight">{{ t('settings.title') }}</h1>
    ...
    >
      ...
      <form class="grid gap-3 sm:grid-cols-2" @submit.prevent="changePassword">
        ...
        <div class="sm:col-span-2">
          <button type="submit" class="rounded-md px-3 py-2 text-[13px] font-medium text-white"
                  :style="{ background: 'var(--series-1)' }">
            {{ t('common.save') }}
          </button>
          <span v-if="pwMessage" class="ml-3 text-[12px]" :style="{ color: 'var(--status-good)' }">
            {{ pwMessage }}
          </span>
          <span v-if="pwError" class="ml-3 text-[12px]" :style="{ color: 'var(--status-critical)' }">
            {{ pwError }}
          </span>
        </div>
      </form>
    ...
    >
      ...
      <p class="mb-3 text-[12px]" :style="{ color: 'var(--ink-secondary)' }">
        {{ session.user.totp_enabled ? t('settings.twofaOn') : t('settings.twofaOff') }}
      </p>

      <div v-if="!session.user.totp_enabled && !totpSecret">
        <button class="rounded-md border px-3 py-2 text-[13px]"
                :style="{ borderColor: 'var(--line-axis)' }" @click="setupTOTP">
          {{ t('settings.twofaSetup') }}
        </button>
      </div>

      <div v-if="totpSecret" class="flex flex-wrap items-start gap-5">
        <img :src="totpQR" alt="QR-Code für die Authenticator-App" width="160" height="160"
             class="rounded-md" />
        <div>
          <p class="mb-2 max-w-xs text-[12px]" :style="{ color: 'var(--ink-secondary)' }">
            {{ t('settings.twofaScan') }}
          </p>
          <code class="tabular mb-3 block text-[11px] break-all" :style="{ color: 'var(--ink-muted)' }">
            {{ totpSecret }}
          </code>
          <div class="flex gap-2">
            <input v-model="totpCode" inputmode="numeric" maxlength="6" placeholder="000000"
                   class="tabular w-28 rounded-md border px-3 py-2 text-center text-[13px]" :style="inputStyle" />
            <button class="rounded-md px-3 py-2 text-[13px] font-medium text-white"
                    :style="{ background: 'var(--series-1)' }" @click="enableTOTP">
      ...
      <p v-if="totpError" class="mt-2 text-[12px]" :style="{ color: 'var(--status-critical)' }">
        {{ totpError }}
      </p>
    </section>
    ...
    >
      ...
      <div class="mb-4 flex gap-2">
        <button
          ...
          :style="{
            borderColor: theme.mode === mode ? 'var(--series-1)' : 'var(--line-axis)',
            color: theme.mode === mode ? 'var(--series-1)' : 'var(--ink-secondary)',
          }"
          @click="setTheme(mode)"
        >
      ...
      <div class="flex gap-2">
        <button
          ...
          :style="{
            borderColor: i18n.locale === locale ? 'var(--series-1)' : 'var(--line-axis)',
            color: i18n.locale === locale ? 'var(--series-1)' : 'var(--ink-secondary)',
          }"
