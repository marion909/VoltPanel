...
async function act(service, action) {
  ...
  error.value = ''
  try {
    const updated = await api.post(`/system/services/${service.name}/${action}`)
    // Nur die betroffene Kachel ersetzen — ein Reload aller Dienste würde die
    // Liste bei jedem Klick springen lassen.
    services.value = services.value.map((s) => (s.name === updated.name ? updated : s))
  } catch (err) {
    error.value = err.message
  } finally {
    pending.value = ''
...
<template>
  <div class="fade-in px-8 py-6">
    <h1 class="mb-5 text-[18px] font-semibold tracking-tight">{{ t('services.title') }}</h1>
      ...
      >
        <div class="flex items-start justify-between gap-2">
          ...
          <span
            ...
            :style="{ color: service.active ? 'var(--status-good)' : 'var(--ink-muted)' }"
          >
            <span
              class="inline-block h-2 w-2 rounded-full"
              :style="{ background: service.active ? 'var(--status-good)' : 'var(--line-axis)' }"
              aria-hidden="true"
            />
        ...
        <div class="mt-3 flex items-center justify-between gap-2">
          <span class="text-[11px]" :style="{ color: 'var(--ink-muted)' }">
            {{ t('services.autostart') }}: {{ service.enabled ? t('common.yes') : t('common.no') }}
          </span>
