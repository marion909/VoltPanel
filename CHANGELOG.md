...
- install.sh verschluckte jeden Startfehler von MariaDB
  ...
  kaputtes MariaDB fiel so erst bei volt db add auf. Jetzt dasselbe
  Muster: Dienststatus prüfen, bei Fehlschlag warnen mit den letzten
  15 Journal-Zeilen
...
- volt tenant set-plan <tenant> 0 hob wie plan remove alle
  ...
  ein echtes Paket zuordnen fragt weiterhin nicht nach
...
- volt tenant import gab bei einem nur teilweise geglückten Import
  ...
  die unvollständigen Zahlen nennt; die Daten liegen wie zuvor
  bereits, nichts wird zurückgerollt
